...
组件分为函数组件和类组件。函数组件是一个返回 jsx 的函数。类组件在 render 方法中返回 jsx。
...
`setup` 中所依赖的 props, state, variable and function 的数组。数组数量要保持不变。 React 使用 `Object.is` 来和旧值比较是否有变化。
...
`useLayoutEffect` 是在 DOM 更新之后，浏览器绘制之前的操作，避免了页面闪烁的问题。
...
```javascript
// context is createdBy createContext api
// value: value passed to the closest context.Provider in component tree.
