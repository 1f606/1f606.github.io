...
### vue 双向绑定的原理是什么
数据劫持加上观察者模式，通过Object.defineProperty劫持变量的get和set，在数据变化时发布消息，通知订阅者，触发监听回调。

Dep Watcher Observer

Observer 数据劫持，Dep 发布消息，依赖手机。Watcher 订阅消息。

首先是对数据进行监听，当监听的属性发生变化时则通知订阅者，订阅者更新就会执行对应的函数从而更新视图
