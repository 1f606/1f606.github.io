...
- ViewModel：视图模型层，通过双向绑定用来连接Model和View。限定 Model 和 View 只能通过 ViewModel 通信。

> MVC 和 MVVM 不同的在于 Controller，监听模型数据的改变和控制视图行为、处理用户交互以及未限制数据流，Model 和 View 之间可以通信。
...
### vue 双向绑定的原理是什么
数据劫持加上观察者模式，通过 `Object.defineProperty` 劫持变量的get和set，在数据变化时发布消息，通知订阅者，触发监听回调。

对数据进行监听，属性被依赖时收集订阅者，当监听的属性发生变化时则通知订阅者，订阅者更新就会执行对应的函数从而更新视图
...
- 实现方式不同。hash模式是依靠hashchange事件，而 history 模式是主要是依靠的 HTML5 history Api
...
### 如何给SPA做SEO优化
1. 服务端渲染，如nuxt
2. 静态化。将动态页面抓取并保存为静态页面或通过服务器的 URL Rewrite，把外部请求的静态地址转化为实际的动态页面地址。
...
减小项目文件体积：使用在线字体和雪碧图。避免重复打包,UI框架按需加载。
...
设置 HTTP 缓存，强制缓存、对比缓存。

区分加载数据优先级 : preload（预先请求当前页面需要的资源） prefetch（将来页面中使用的资源）将数据缓存到 HTTP 缓存中
...
减少 DOM 操作，缓存需要多次访问的元素

requestAnimationFrame、requestIdleCallback（帧空闲时间执行）
...
### 对于 async、defer 的理解
由于 html 的解析是由上到下的，当遇到 script 标签时就会停止解析，下载 js 然后执行，执行完毕后继续解析，所以我们需要把 script 标签放到 html 的最下方，等到 dom 解析完毕，再去执行 js，才能实现 hmtl 的解析不被 js 阻塞。

为了解决 script 标签必须放到底部的问题，那么可以对 script 标签添加 async、defer 属性。

两者区别：async 并行下载，立即执行。defer 并行下载，html 构建完成后，DOMContentLoaded 触发前执行。

#### defer

当浏览器解析遇到带有 defer 标签的 script 时，会继续往下面解析页面，并行下载 script 标签引入的外部 js 文件，HTML 构建完成后，再执行刚下载的 js 脚本。

具体是 DOMContentLoaded 事件调用前执行，执行顺序是 defer 标签在页面上的先后顺序。

DOMContentLoaded 事件的触发时机为初始 HTML 被构建完成时，此时 CSS、图片等资源不需要加载完，但脚本要执行完。

内联 js 代码的以及动态生成的 script 标签，defer 属性不生效。

#### async

当浏览器解析遇到带有 async 属性的 script 标签，会继续往下解析，并且同时另开进程下载脚本。

脚本下载完毕，浏览器停止解析，立即执行脚本，执行完毕后再继续往下解析。
...
### 虚拟DOM
虚拟DOM本质就是用一个 JavaScript 对象去描述一个 DOM 节点。是对真实 DOM 的一层抽象。

由于在浏览器中操作频繁的操作DOM，会产生一定的性能问题，因此我们需要这一层抽象，在patch过程中尽可能地一次性将差异更新到DOM中，这样保证了DOM不会出现性能很差的情况。

另外是为了更好的跨平台，比如 Node.js 就没有 DOM,如果想实现服务端渲染,那么就可以借助 Virtual DOM。
...
### 层叠上下文
当元素的 z-index 为数值并且生效的时候，会使得容器内的子组件无法穿过容器本身，并且子组件的层级由父组件决定。这种变化后的容器元素我们称之为层叠上下文。

特性：
- 层叠上下文可以嵌套，内部层叠上下文及其所有子元素均受制于外部的层叠上下文。
- 每个层叠上下文和兄弟元素独立，也就是当进行层叠变化或渲染的时候，只需要考虑后代元素。
- css3创建的层叠上下文，其层叠等级与 z-index : 0 相同。

创建方式：
- 页面根元素
- z-index 值为数值的定位元素
> z-index: auto 和 z-index: 0 在层叠等级上属于同一级。但是 z-index: 0 会使得元素升级为层叠上下文。
- CSS3 属性。
...
两个概念：
- 层叠顺序（ Stacking Order ），它表示元素发生层叠时在 Z 轴上垂直显示顺序。
- 层叠等级（ Stacking Level ），层叠等级决定了同一个层叠上下文中元素在 Z 轴上的显示顺序。

层叠规则：
- 当互相层叠的元素都是层叠上下文元素，即明确的有 z-index 值的时候（没有明确的 z-index 值的层叠上下文 z-index 值为0 ）直接比较 z-index 的值，值大的处在值小的元素的上方。
...
#### 双飞翼布局、圣杯布局
双飞翼和圣杯布局三列布局，左右两列定宽，中间列自适应剩余宽度。

中间列书写在前保证提前渲染，三个元素都是浮动，中间宽度都设置百分百。

双飞翼：左列margin-left -100%（以父元素为基准），右列margin-left-自己宽度。中间的子元素通过设置margin或padding空出两边。

圣杯：左列margin-left -100%，relative，left -自己宽度，右列right和margin-left -自己宽度，父元素padding空出两边，
...
https://juejin.cn/post/6973555813181259783?share_token=8098d19b-8b39-4e67-ba88-f3cf6bd60215
...
- :only-of-type 选择属于其父元素唯一的一种标签元素
...
### 如何理解 line-height
行高是指两行文字间基线的距离。CSS中起高度作用的是height和line-height，没有定义height属性，那撑起高度的一定是line-height。

### 怎么让 Chrome 支持小于12px 的文字？
transform scale

### 让页面里的字体变清晰，变细用 CSS 怎么做？
在 IOS 上可以设置 -webkit-font-smoothing：antialiased
...
## JS
### 闭包是什么? 闭包的用途?
闭包是函数及其词法环境的组合。闭包会返回一个函数，只要外部还维持着对函数的的引用，内部的词法环境就不会被回收。开发者就可以在外部借助返回的函数访问
函数内的局部变量。

### 事件循环
[事件循环](https://4sq.me/2023/02/01/%E6%B5%8F%E8%A7%88%E5%99%A8%E4%BA%8B%E4%BB%B6%E5%BE%AA%E7%8E%AF/)

### CommonJS 和 ES6 模块的区别
- CommonJS 是同步加载，同步的意味着只有等到对应的模块加载完毕，当前模块中的内容才能被运行。加载的是 module.exports 对象，exports 对象只是 module.exports 的一个引用。ES Module加载js文件的过程是编译时加载的，并且是异步的。
- ES Module 可以加在 CommonJS 模块，后者不能加载前者。
- CommonJS 模块的文件只会被执行一次，无论被导入几次。
...
- 在隐藏或不可见的元素中，requestAnimationFrame 将不会进行重绘或回流。如果页面不是激活状态下的话，动画会自动暂停。
...
### 常用继承方案
#### 寄生组合式继承
目前最成熟的方法。子构造调用父构造，子构造原型对象指向父构造原型对象（Object.create）
...
#### 混入方式继承多个对象
子构造调用需要被继承的构造函数，子构造原型对象指向被继承构造的原型对象(Object.create)，其他需要继承的通过 `Object.assign` 赋值到子构造原型对象。
...
2. .then, .catch 和 .finally都会返回一个新的Promise。finally 返回的默认会是一个promise对象，值是上一次的Promise值，不过如果抛出的是一个异常则值是返回异常的Error对象。
3. catch不管被连接到哪里，都能捕获上层未捕捉过的错误。
4. 在Promise中，返回任意一个非 promise 的值都会被包裹成 promise 对象，例如return 2会被包装为return Promise.resolve(2)。 
5. Promise 的 .then 或者 .catch 可以被调用多次, 但如果Promise内部的状态一经改变，并且有了一个值，那么后续每次调用.then或者.catch的时候都会拿到该值。
6. .then 或者 .catch 中 return 一个 error 对象并不会抛出错误，所以不会被后续的 .catch 捕获。
7. .then 或 .catch 返回的值不能是 promise 本身，否则会造成死循环。
8. .then 或者 .catch 的参数期望是函数，传递的是非函数，它实际上会将其解释为then(null)，这就会导致前一个Promise的结果会传递到下面。
9. .then方法是能接收两个参数的，第一个是处理成功的函数，第二个是处理失败的函数。和 catch 的区别就是，如果它和 catch 同时存在，promise 中的报错只会被 then 第二个函数捕获。
10. .finally 在Promise结束的时候，无论结果为resolved还是rejected，都会执行里面的回调函数。
11. .finally()方法的回调函数不接收任何的参数，在.finally()函数中是没法知道Promise最终的状态是resolved还是rejected的
...
- 变量声明了未赋值或函数被调用时未传入指定参数，值就等于 `undefined`。
- 函数没有返回值时，默认为 `undefined`
...
- 减少请求数量和体积
- 将内容提升到它们自己的层从而提高渲染性能。但是它以内存管理为代价，不能过度使用。有一些特定的属性和元素可以实例化一个层，包括 `<video>` 和 `<canvas>`，任何 CSS 属性为 opacity 、3D transform、will-change 的元素。

#### 回流（重排）和重绘
回流会触发重新绘制和重新排列。由于DOM的结构发生了改变，所以需要从生成DOM这一步开始，重新经过样式计算、生成布局树、建立图层树、再到渲染到显示器这整一个渲染过程走一遍，开销是非常大的。

什么时候会触发回流呢？一般来说，当元素位置发生变化时就会。不过一般浏览器会自动合并更改，在达到某个数量或时间后，会合并为一次回流。
...
* 客户端生成对称秘钥，然后使用服务端公钥加密对称秘钥，再发送加密后的对称秘钥。
* 后续通过对称秘钥加密进行通信
...
它是由一些数字认证机构（CA）颁发给服务器的一个文件。至于颁发证书的流程，简单来说是：

* 认证机构给明文信息(申请者的公钥，组织和认证机构等信息)进行数字签名，过程也就是签名提到的数字签名的步骤：1.通过Hash函数处理明文信息生成一个信息摘要；2.再用认证机构自己的私钥对信息摘要进行加密处理。通过这两个步骤生成的文件就叫数字签名。
...
在过去，前端主要编写html、css和js代码。随着Web应用程序的复杂性和规模的不断增长，简单的手动编码无法满足现代开发的要求。这就有了前端工程化的概念。

前端工程化可以被定义为使用工具和最佳实践来提高前端开发效率、质量和可维护性的方法。它涉及到构建工具、模块化、自动化测试、性能优化、版
本控制等方面的实践，旨在提供一种可靠、高效的开发环境，并使团队能够更好地协作。

https://juejin.cn/post/7250730877372645431
...
// async写法 在语言层面实现了generator自动执行器
...
即时通讯等实时性要求高的场景，就需要用 websocket。

websocket 是和 http 不同的协议。但是需要一次从 http 到 websocket 的切换过程。
...
Sec-WebSocket-Key 经过一定处理后作为 Sec-WebSocket-Accept 返回。
...
function hashKey(key) {
  const sha1 = crypto.createHash('sha1');
  // 固定的值
  const constantKey = '258EAFA5-E914-47DA-95CA-C5AB0DC85B11';
  sha1.update(key + constant);
  return sha1.digest('base64');
}
```

![img.png](/img/websocket示意图.png)

协议的第一个字节存储了FIN（结束标志），opcode（内容类型），第二个字节存储了 mask（是否有加密），payload（数据长度）。
...
这段代码会输出 ƒ b() { b = 20; console.log(b); }。这是因为在这个立即执行函数表达式 (IIFE) 内部，函数 b 被声明为一个函数表达式，并且函数的名称会覆盖外部的变量 b。

在函数体内，有一行 b = 20; 试图将函数 b 赋值为 20，但是由于在函数体内，b 已经是当前函数的引用了，函数名是只读的。因此，这行代码实际上没有效果。
...
因为动态添加的src，编译过后的文件地址和被编译过后的资源文件地址不一致，从而无法正确引入资源。而使用require，返回的就是资源文件被编译后的文件地址，
从而可以正确的引入资源
...
https://mp.weixin.qq.com/s/HfN-ULy9KD86vTuzDDFIiw
