...
4. 父级div定义zoom
5. 父元素伪元素设置，前后伪元素 display table, 后伪元素clear both
...
### 浏览器的渲染
浏览器渲染大致分为五步，创建 DOM 树，创建 styleRules，创建 render 树，布局 layout 和绘制 Painting。
1. 分析 HTML 元素，构建一颗 DOM 树。
2. 分析 css 文件和元素内联样式，生成页面样式表。
3. 将 DOM 树和样式表关联起来，构建一颗 render 树。
4. 浏览器根据 render 树开始布局，确定每个节点的坐标。
5. 根据 render 树和节点坐标，绘制节点。

DOM 树的构建是一个渐进过程，不必等到文档加载完成才开始。

render树也不是等到 DOM 树和 cssom 树构建完毕才开始构建。三者互有交叉，一边加载，一边解析，一边渲染。
