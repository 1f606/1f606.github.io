...
Date: toJSON()
对象：1.toJSON() 2.symbol为key的属性会被忽略 3. 值为 symbol, function 和 undefined的会被忽略
...
function thousandFormat (v) {
  const validNumber = parseFloat(v);
  if (isNaN(validNumber)) return '';
  const numArr = String(validNumber).split('.');
  const result = [];
  for (var i = numArr[0].length - 1; i >= 0; i--) {
    if (i % 3 === 0 && i !== numArr[0].length - 1) {
      result.unshift(',');
    }
    result.unshift(numArr[0][i]);
  }
  if (numArr[1]) result.push(...numArr[1]);
  return result.join('');
}
```
